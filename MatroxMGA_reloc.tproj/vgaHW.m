...
//      (*vgaSaveScreenFunc)(SS_START);		/* start synchronous reset */
      vgaHWSaveScreen(SS_START);		/* start synchronous reset */
      outw(0x3C4, ((tmp | 0x20) << 8) | 0x01);	/* disable the display */

      tmp = BREG (vgaIOBase + 0x0A);
      BREGO (0x3C0, 0x00);			/* enable pallete access */
    }
    else {
      /*
...
//      (*vgaSaveScreenFunc)(SS_FINISH);		/* clear synchronousreset */
      vgaHWSaveScreen(SS_FINISH);		/* clear synchronousreset */

      tmp = BREG (vgaIOBase + 0x0A);
      BREGO (0x3C0, 0x20);			/* disable pallete access */
    }
//  }
...
Bool vgaBlankScreen(unsigned char *MGAMMIOBase, ScreenPtr ptr, Bool on)
{
    ...
    int PowerManagementMode;
{
  ...
  BREGO (0x3C5, seq1);
  BREGO (vgaIOBase+4, 0x17); /* Select CRTC17 */
  crtc17 |= BREG (vgaIOBase+5) & ~0x80;
  usleep(10000);
  BREGO (vgaIOBase+5, crtc17);
...
#if !defined(PC98_NEC480) && !defined(PC98_EGC)
  tmp = BREG (vgaIOBase + 0x0A);		/* Reset flip-flop */
  BREGO (0x3C0, 0x00);			/* Enables pallete access */
#endif
...
//  vgaSaveScreen(NULL, FALSE);
  vgaBlankScreen(MGAMMIOBase, NULL, FALSE);
  ...
  if(restore->FontInfo1 || restore->FontInfo2 || restore->TextInfo) {
     ...
     */
    tmp = BREG (vgaIOBase + 0x0A); /* reset flip-flop */
    BREGO (0x3C0,0x30); BREG (0x3C0, 0x01); /* graphics mode */
#ifdef XF86VGA16
...
//  vgaSaveScreen(NULL, TRUE);
  vgaBlankScreen(MGAMMIOBase, NULL, TRUE);
  ...
  BREGO (0x3C2, restore->MiscOutReg);
  ...
  for (i=0; i<21; i++) {
    tmp = BREG (vgaIOBase + 0x0A);
    BREGO (0x3C0,i); BREGO (0x3C0, restore->Attribute[i]);
  }
...
- (void *)vgaHWSave:(vgaHWPtr)save withSize:(int)size
{
  ...
  if (save == NULL) {
    ...
    if (vga256InfoRec.clockprog && vga256InfoRec.textclock >= 0)
      ...
      save->NoClock = (BREG (0x3CC) >> 2) & 3;
#endif /* !defined(PC98_NEC480) && !defined(PC98_EGC) */
  }

#if !defined(PC98_NEC480) && !defined(PC98_EGC)
  save->MiscOutReg = BREG (0x3CC);
#ifdef PC98
  ...
  if (first_time)
  {
     ...
     */
    BREGO (0x3C7,0x00);
    for (i=0; i<3; i++) save->DAC[i] = BREG (0x3C9);
#if defined(PC98_PW) || defined(PC98_XKB) || defined(PC98_NEC) || defined(PC98_PWLB)
    for (i=0; i<300; i++) BREG (vgaIOBase + 4);
#endif
    BREGO (0x3C8,0x00);
    for (i=0; i<3; i++) BREGO (0x3C9, ~save->DAC[i] & vgaRamdacMask);
#if defined(PC98_PW) || defined(PC98_XKB) || defined(PC98_NEC) || defined(PC98_PWLB)
    for (i=0; i<300; i++) BREG (vgaIOBase + 4);
#endif
    BREGO (0x3C7,0x00);
  ...
  for (i=0; i<25; i++) { BREGO (vgaIOBase + 4,i);
			 save->CRTC[i] = BREG (vgaIOBase + 5); }

  for (i=0; i<21; i++) {
    tmp = BREG (vgaIOBase + 0x0A);
    BREGO (0x3C0,i);
    save->Attribute[i] = BREG (0x3C1);
  }

  for (i=0; i<9;  i++) { BREGO (0x3CE,i); save->Graphics[i]  = BREG (0x3CF); }

  for (i=0; i<5;  i++) { BREGO (0x3C4,i); save->Sequencer[i] = BREG (0x3C5); }
...
//  vgaSaveScreen(NULL, FALSE);
  ...
  BREGO (0x3C2, save->MiscOutReg | 0x01);	/* shift to colour emulation */
  ...
  if (((save->Attribute[0x10] & 0x01) == 0) && (size >= 0)) {
    ...
    if (save->FontInfo1 == NULL) {
       ...
       * BUGALLERT: The vga's segment-select register MUST be set appropriate !
       */
      tmp = BREG (0x3D0 + 0x0A); /* reset flip-flop */
      BREGO (0x3C0,0x30); BREGO (0x3C0, 0x01); /* graphics mode */
      outw(0x3C4, 0x0402);    /* write to plane 2 */
      outw(0x3C4, 0x0604);    /* enable plane graphics */
      outw(0x3CE, 0x0204);    /* read plane 2 */
      outw(0x3CE, 0x0005);    /* write mode 0, read mode 0 */
  ...
  BREGO (0x3C2, save->MiscOutReg);		/* back to original setting */
#endif /* !defined(PC98_NEC480) && !defined(PC98_EGC) */
...
- (BOOL)vgaHWInit:(DisplayModePtr)mode size:(int)size
{
  ...
  if (vgaNewVideoState == NULL) {
    ...
    vgaNewVideoState = (void *)IOMalloc(size);	/* (1,size) */
	bzero(vgaNewVideoState, size);
...
//    new->NoClock = mode->Clock;
...
#ifdef DEBUG
	ErrorF("clock=%d, noclock=%d\n", mode->Clock, new->NoClock);
#endif
  ...
  BREGO (0x6a, 0x01);
  /* set EGC enable */
  BREGO (0x7c, 0xc0); /* GRCG enable, RMW mode */
  BREGO (0x6a, 0x07);
  BREGO (0x6a, 0x05);
  BREGO (0x6a, 0x06);
